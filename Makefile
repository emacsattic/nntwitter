...
--eval "(custom-set-variables \
  (quote (gnus-select-method (quote (nntwitter \"\")))) \
...
test-run:
	$(CASK) emacs -Q --batch \
	  $(TESTRUN) \
	  --eval "(require 'nntwitter)" \
	  --eval "(cl-assert (nntwitter-rpc-get))" \
	  --eval "(sleep-for 0 7300)" \
	  -f nntwitter-dump-diagnostics \
	  --eval "(cl-assert nntwitter-processes)"
